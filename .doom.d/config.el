...
;; Initialize euporie-termint with manual reload fallback
(if (and (featurep 'euporie-termint)
    ...
    (progn 
      ...
      (message "euporie-termint: Setup completed during startup"))
  (progn
    (message "euporie-termint: Functions not available during startup, setting up manual reload...")
    ;; Force manual reload after init to fix function availability
    (add-hook 'after-init-hook 
              (lambda ()
                (message "euporie-termint: Attempting manual reload...")
                (load (expand-file-name "euporie-termint.el" (or doom-user-dir "~/.doom.d/")))
                (when (and (fboundp 'euporie-termint-setup) 
                          (fboundp 'euporie-termint-setup-keybinding))
                  (euporie-termint-setup)
                  (euporie-termint-setup-keybinding)
                  (message "euporie-termint: Setup completed via manual reload"))))))
